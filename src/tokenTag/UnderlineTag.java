...
public class UnderlineTag extends TokenTag {
	
	public UnderlineTag(StyledDocument target, String start, String end) {
		super(target, start, end);
		
        Style def = StyleContext.getDefaultStyleContext().
                getStyle(StyleContext.DEFAULT_STYLE);
		Style s = doc.addStyle("underline", def);
	...
	public void apply() throws BadLocationException {
		...
		for(int i=str.indexOf(this.startToken), j=str.indexOf(this.endToken, i+1); 
			...
			doc.setCharacterAttributes(i, j-i+1, this.doc.getStyle("underline"), true);
